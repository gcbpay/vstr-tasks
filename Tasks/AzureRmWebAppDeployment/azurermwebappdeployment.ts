...
async function run() {
    ...
    tl.debug("updating scm information");
    await updateScmTypeForWebApp(SPN, resourceGroupName, webAppName, deployToSlotFlag, slotName);
}
...
async function updateScmTypeForWebApp(SPN, resourceGroupName: string, webAppName: string, deployToSlotFlag: boolean, slotName: string) {
    try {
